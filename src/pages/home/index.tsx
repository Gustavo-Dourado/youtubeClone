...
function Home(){
    ...
    async function getVideosYoutube(){
        ...
        const url = `https://www.googleapis.com/youtube/v3/videos?part=snippet&part=statistics&chart=mostPopular&h1=pt-BR&maxResults=24&regionCode=BR&videoCategoryId=${categoryId}&key=${API_KEY}`

        try{
            const response = await axios.get(url)
            setYoutubeArray(response.data.items)
        } catch(error){ 
            console.log(error)
        }
    }
